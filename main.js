...
function togglePastAct() {
    ...
    if (pastActShow === true) {
        pastActShow = false;
        hideActs.style("display", "none");
    } else {
        pastActShow = true;
        hideActs.style("display", "flex");
    }
}
function updateDir() {
    fs
        .writeFile("setup.txt", document.getElementById("newDir").value, function (err) {
            if (err) {
...
function newAct(action1, action2) {
    if (playing) {
        document
            .getElementById("p1ActText")
            .textContent = action1 + " " + document
            .getElementById("p1ActText")
            .textContent;
        document
            .getElementById("p2ActText")
            .textContent = action2 + " " + document
            .getElementById("p2ActText")
            .textContent;
    }
}
...
function filterCombos() {
    ...
    if (dropdownIndex == 0) {
        ...
        var didKill = document
            .getElementById("didKill")
            .checked;
...
function players() {
    ...
    if (actionStates != undefined && showAction) {
        ...
        if (pastActShow) {
            newAct(actionStates[p1State], actionStates[p2State]);
        }
    }
...
function stickStuff() {
    ...
    let p1JY;
    let p2JY;
    p1JY = frames[currentFrame].players[0].pre.joystickY;
    p1JY *= 10;
    p1JY += 20;
    p1JY += "px";
    document
        .getElementById("p1Stick")
        .style
        .top = p1JY;
    p1JX = frames[currentFrame].players[0].pre.joystickX;
    p1JX *= 10;
    p1JX += 20;
    p1JX += "px";
    document
        ...
        .style
        .left = p1JX;
    p1JY = frames[currentFrame].players[0].pre.cStickY;
    p1JY *= 10;
    p1JY += 40;
    p1JY += "px";
    document
        .getElementById("p1CStick")
        .style
        .top = p1JY;
    p1JX = frames[currentFrame].players[0].pre.cStickX;
    p1JX *= 10;
    p1JX += 100;
    p1JX += "px";
    document
        ...
        .style
        .left = p1JX;
    p2JY = frames[currentFrame].players[1].pre.joystickY;
    p2JY *= 10;
    p2JY += 20;
    p2JY += "px";
    document
        .getElementById("p2Stick")
        .style
        .top = p2JY;
    p2JX = frames[currentFrame].players[1].pre.joystickX;
    p2JX *= 10;
    p2JX += 20;
    p2JX += "px";
    document
        ...
        .style
        .left = p2JX;
    p2JY = frames[currentFrame].players[1].pre.cStickY;
    p2JY *= 10;
    p2JY += 40;
    p2JY += "px";
    document
        .getElementById("p2CStick")
        .style
        .top = p2JY;
    p2JX = frames[currentFrame].players[1].pre.cStickX;
    p2JX *= 10;
    p2JX += 100;
    p2JX += "px";
    document
...
function buttonLogic() {
    let buttonString = frames[currentFrame]
        .players[0]
        .pre
        .physicalButtons
        .toString(2);
    ...
    /*if(buttonString.charAt(6)=='1'){
		document.getElementById("p1ZButton").style.background="#60f";
	}
	else{
		document.getElementById("p1ZButton").style.background="#1d1a1a";
	}
